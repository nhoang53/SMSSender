...
public class MessageActivity extends AppCompatActivity {
        ...
        if(requestCode == REQUEST_CODE_ADD_CONTACT && resultCode == Activity.RESULT_OK)
        {
            ...
            if(cursor.moveToFirst())
            {
                String  name = cursor.getString(cursor.getColumnIndexOrThrow(
                        ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME)); // we don't know field name, so use what Google design
                ...
                //contactsAdapter.notifyDataSetChanged(); // don't really need this case
            }
            cursor.close();
        }
    }
    ...
    public void sendTextMessage(View view) {
        ...
        if(ActivityCompat.checkSelfPermission(this, Manifest.permission.SEND_SMS) != PackageManager.PERMISSION_GRANTED)
        {
            ...
            //Define a reference to SmsManager(manager text message)
